...
void *run(void* socket_file_descr_new);
int main(int argc, char *argv[]) {
	if (!argc < MIN_ARGS) {
		...
		while (1) {
			...
			pthread_t thread;
			int ret;
			ret = pthread_create(&thread, NULL, run,
					(void*) &socket_file_descr_new);
			if (ret < 0) {
				signal_error("Failed to create thread");
			}
			//joining thread is not required as the parent never exits
		}
	} else {
		signal_error(
...
void *run(void* socket_file_descr_new) {
	//printf("hello %d",*((int *)socket_file_descr_new));
	//printf("hello");
	processRequest(*(int *) socket_file_descr_new, FALSE);
	return 0;
}
...
/**
 * Reads HTTP request from the socket and evaluates/parses the request
 */
